...
resource "aws_iam_role" "cluster" {
  name               = "${var.name_prefix}-role-eks-cluster-01"
...
resource "aws_security_group" "cluster" {
  name        = "${var.name_prefix}-sg-eks-01"
  description = "Cluster communication for ${var.cluster_name}"
  vpc_id      = var.vpc_id
...
resource "aws_iam_role" "node" {
  name               = "${var.name_prefix}-role-eks-node-01"
...
# Grant additional IAM principals (e.g. the self-hosted Jenkins agent role)
# cluster-admin access so they can run kubectl against the cluster.
